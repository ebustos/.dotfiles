...
if !exists("*GetErlangFold")
	...
	let s:ErlangFunHead     = '^\a\w*(.*)\(\s+when\s+.*\)\?\s\+->\s*$'
        " NOTE: Added by Emilio
	let s:ErlangRecEnd      = '^$'
	let s:ErlangRecHead     = '^-record.*'
        " 
	...
	function GetErlangFold(lnum)
		...
		if line=~ s:ErlangBeginHead
			...
			return '1>'
                ...
                " Note: Added by Emilio
		if line=~ s:ErlangRecHead
			while line !~ s:ErlangRecEnd
				if 0 == lnum " EOF / BOF
					retun '='
				endif
				if line =~ s:ErlangRecEnd
					return '='
				endif
				let lnum = s:GetNextNonBlank(lnum)
				let line = getline(lnum)
			...
			let lnum = s:GetPrevNonBlank(a:lnum)
			if exists("g:erlangFoldSplitFunction") && g:erlangFoldSplitFunction
				if getline(lnum) !~ s:ErlangRecEnd
					return '='
				endif
			endif
			return '1>'
		...
		if line =~ s:ErlangFunEnd || line =~ s:ErlangRecEnd
			return '<1'
		endif
