...
syn keyword pgsqlKeyword	 cache conversion cascade current_date current_time current_timestamp class close
syn keyword pgsqlKeyword	 cluster checkpoint check comment
syn keyword pgsqlKeyword	 cast cascade character check column columns commit constraint create copy
...
syn keyword pgsqlKeyword	 increment index into immutable inner initially immediate inherits instead insert in if
...
syn keyword pgsqlKeyword	 select set start strict schema sequence savepoint simple system stable setof
...
"syn region pgsqlString		 start=+"+  skip=+\\\\\|\\"+  end=+"+ contains=all
"syn region pgsqlString		 start=+'+  skip=+\\\\\|\\'+  end=+'+ contains=ALL
...
syn keyword pgsqlType		 void xid 
...
syn region pgsqlType		 start="\Wvarchar(" end=")" contains=pgsqlNumber,pgsqlVariable
